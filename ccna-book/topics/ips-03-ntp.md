...
## Кому ещё нужно общее время

Журналы — самый заметный, но не единственный потребитель синхронного времени:

- **IP SLA** с операцией `udp-jitter` считает джиттер по меткам времени, проставленным на
  **двух разных устройствах** — без NTP измерение теряет смысл (поэтому в вопросе «что
  требуется для измерения UDP jitter» ответ — NTP, а не CDP/LLDP/EEM).
- **Сертификаты** (SSH, HTTPS, 802.1X с EAP-TLS) проверяются по сроку действия: уехавшие
  часы дают «сертификат ещё не действителен» на исправном сертификате.
- **Kerberos и AAA** отвергают запросы с расхождением времени больше допуска.
- **Расписания**: `time-range` в ACL и планировщик задач срабатывают не в то время.
