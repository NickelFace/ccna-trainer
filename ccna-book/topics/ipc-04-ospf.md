...
Стоимость решает только **внутри OSPF**. Когда в таблицу претендуют маршруты из разных
источников, порядок другой и его спрашивают вместе с exhibit'ом «как роутер обработает
трафик к такому-то адресу»:

1. **Longest prefix match** — сначала выбирается самый длинный подходящий префикс; /28 из
   OSPF победит /24 из статики просто потому, что он конкретнее.
2. Если префиксы одинаковой длины — **административное расстояние**: connected 0, static
   1, eBGP 20, EIGRP 90, **OSPF 110**, RIP 120.
3. И только при равном AD (то есть внутри одного протокола) — метрика, для OSPF это cost.

Подробный разбор с выводом `show ip route` — в главе про таблицу маршрутизации.
...
Отдельно спрашивают, **чем смотрят саму базу состояний каналов**: это `show ip ospf
database` — список LSA, то есть общая для области топология, из которой SPF считает
маршруты. Не путать с `show ip ospf neighbor` (соседи) и `show ip route ospf` (уже
посчитанные маршруты).

Таймеры при необходимости правят на интерфейсе, и менять их надо **на обеих сторонах**:
