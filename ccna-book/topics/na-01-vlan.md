...
> [!trap] Ловушка
> В вариантах ответа регулярно попадаются команды **private VLAN** — `switchport
> private-vlan association host`, `vlan 10 / private-vlan isolated`. Private VLAN — это
> отдельная технология «VLAN внутри VLAN» (порты **isolated** не видят друг друга,
> **community** видят только своих, **promiscuous** видит всех; применяется в хостингах и
...
Формы команды `switchport voice vlan`, которые ставят вариантами ответа:

| Форма | Что говорит телефону |
|---|---|
| `switchport voice vlan 20` | голос — **в отдельной VLAN 20**, тегированный. Это и есть стандартный ответ, когда в задаче названа голосовая VLAN |
| `switchport voice vlan dot1p` | голос — **в VLAN 0**: тег есть только ради приоритета CoS, номера VLAN нет |
| `switchport voice vlan untagged` | голос — **без тега**, вместе с данными в той же access VLAN |
| `switchport voice vlan none` | телефону ничего не сообщается, он использует свои настройки |

Рядом стоит вторая команда — **`switchport priority extend`**, и она уже про то, как
телефон обходится с трафиком **компьютера, включённого в его порт**:

- `switchport priority extend cos 7` — перемаркировать трафик ПК указанным CoS;
- `switchport priority extend trust` — **доверять** маркировке, которую ПК проставил сам,
  и передавать её без изменений. Это ответ на формулировку «порт должен передавать пакеты
  данных с тем же приоритетом, с каким они пришли от устройства за телефоном».
