...
Транк нужен не только между коммутаторами. Два частых случая из банка вопросов: порт к
**distribution-порту WLC** — контроллер отдаёт в сеть клиентский трафик нескольких WLAN,
каждую в своей VLAN, и транк даёт им путь одним линком; порт к **автономной точке
доступа**, которая одновременно несёт клиентскую VLAN и собственный управляющий трафик, —
тоже транк (точке в lightweight/local mode достаточно access-порта, весь её трафик уходит
в CAPWAP одной VLAN).
...
именно тот протокол, который образует **data plane фабрики SD-Access (SDA)** между edge-узлами
