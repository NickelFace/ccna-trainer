...
# The splash screen is not the launcher. Its icon is masked to a *circle* of the inner two
# thirds — radius 36 on this canvas — and a 70x66 glyph inset by 20% has a half-diagonal of
...
# ------------------------------------------------------- Android drawables --
def vector_drawable(colors, comment, inset=INSET):
    paths = "\n".join(
        f'    <path\n        android:fillColor="{c}"\n'
        f'        android:pathData="{rounded_rect_path(x, y, w, h, r)}" />'
        for (x, y, w, h, r), c in zip(fitted_steps(inset=inset), colors)
    )
    return (
...
def main():
    ...
    # The splash sits on the app's own dark page colour, not on the icon's ink plate, so the
    # mark takes its inverted colourway. Same geometry as the launcher: whatever the system
    # does to the splash icon, the silhouette is already inside a 20% margin.
    (RES / "drawable" / "ic_splash.xml").write_text(
        vector_drawable(
            [BONE, BONE, GOLD],
            "Splash glyph: the NetPath mark on the app's page colour. One vector replaces\n"
            "  the twelve rasterised splash.png files Capacitor generates. Inset further than\n"
            "  the launcher because the splash API masks to a circle, not a squircle.\n"
