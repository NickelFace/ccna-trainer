...
//
// The spec asks for one place that owns this formula, so here it is, stated plainly:
//
...
export function readinessDelta(attempts, byN, domains, sinceMs, opts = {}) {
  const now = readiness(attempts, byN, domains, opts);
  if (now.forecast === null) return null;
  const older = attempts.filter(a => a.date <= sinceMs);
  if (!older.length) return null;
  const before = readiness(older, byN, domains, opts);
